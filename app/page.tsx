...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Menyesuaikan dengan response API sesuai dengan dokumentasi fungsi lib/api.ts
        setStats({ totalPatients: countRes.data.total_patients || 0 });
        setRecentData(recentRes.data.data || []);
      } catch (err) {
        console.error("Gagal mengambil data dashboard", err);
  ...
  const scaleAnalysis = useMemo(() => {
    ...
    return [
      { name: 'Skala 1', value: counts['1'], color: '#3b82f6' }, // Ringan (Blue)
      { name: 'Skala 2', value: counts['2'], color: '#10b981' }, // Normal/Awal (Emerald)
      { name: 'Skala 3', value: counts['3'], color: '#f59e0b' }, // Sedang (Amber)
      { name: 'Skala 4', value: counts['4'], color: '#f97316' }, // Parah (Orange)
      { name: 'Skala 5', value: counts['5'], color: '#ef4444' }, // Sangat Parah (Red)
    ];
  }, [recentData]);
